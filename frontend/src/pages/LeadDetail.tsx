        ...
        <div style={{maxWidth:900,margin:'0 auto'}}>
          ...
          <div className="card" style={{marginBottom:12,display:'flex',alignItems:'center',gap:14}}>
            ...
            <div style={{flex:1}}>
              ...
              <div style={{fontSize:12,color:'var(--text2)',marginBottom:2}}>{lead.business_type}{lead.city?` · ${lead.city}`:''}{lead.source?` · ${lead.source}`:''}</div>
              <div style={{fontSize:11,color:'var(--text3)'}}>{lead.phone}{lead.whatsapp && lead.whatsapp !== lead.phone ? ` · WA: ${lead.whatsapp}` : ''}</div>
            </div>
            <div style={{display:'flex',flexDirection:'column',gap:5,flexShrink:0}}>
              <button className="btn btn-primary btn-sm" onClick={() => setTab('call')}>+ Log Call</button>
              <button className="btn btn-ghost btn-sm" onClick={() => setTab('whatsapp')}>+ WhatsApp</button>
            </div>
          </div>
          ...
          {/* MAIN GRID: left = activity, right = profile + greenfield */}
          <div style={{display:'grid',gridTemplateColumns:'1fr 300px',gap:12,alignItems:'start'}}>
            ...
            <div>
              ...
              {tab==='timeline' && (
                allInteractions.length === 0
                  ? <div style={{fontSize:12,color:'var(--text3)',padding:'10px 0'}}>No interactions yet. Log a call or add a WhatsApp chat.</div>
            ...
            {/* RIGHT — profile + score bars + greenfield */}
            <div>
